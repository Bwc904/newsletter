...
1. `pip install -r requirements.txt` — install deps (idempotent).
2. `python newsletter.py` — Grok API call, produces `newsletter.json`. If exit code is non-zero, abort and proceed to step 7 (failure email).
3. `python render.py` — produces `newsletter.html` (the full dark-mode design).
4. `python publish.py` — copies `newsletter.html` into `docs/{date}.html` + `docs/index.html`, regenerates `docs/archive.html`, and produces a minimal `email.html` for delivery.
5. Commit and push the new `docs/` entry to `main` so GitHub Pages serves it:
   ```
   ...
   ```
6. `python send_email.py` — sends `email.html` via Resend API to `$NEWSLETTER_TO`.
7. If any step failed, send a plain-text failure email via Resend with subject `Daily Brief -- FAILED <date>` containing the last 30 lines of stderr. Do not retry automatically.
